...
::: {.callout-note title="Possible mapping identifiers based on mapping samenes identifiers"}
```
...
- [`sssom-pydantic`](https://github.com/cthoyt/sssom-pydantic) is an implementation of SSSOM in Python that contains an implementation of the Mapping Sameness Identifier (see [usage](https://sssom-pydantic.readthedocs.io/en/latest/api/sssom_pydantic.api.hash_triple.html))
- [`jskos-tools`](https://www.npmjs.com/package/jskos-tools) is a JavaScript library that contains an implementation of the Mapping Sameness Identifier (see [usage](https://www.npmjs.com/package/jskos-tools#mappingsamenessidentifier))

<!-- [`SSSOM-Java`](https://github.com/gouttegd/sssom-java/) is an implementation of SSSOM in Java that contains an implementation of the Mapping Sameness Identifier (see [javadocs](https://incenp.org/dvlpt/sssom-java/apidocs/org/incenp/obofoundry/sssom/MappingHasher.html)) -->


## Scripts {-}

The following sample scripts expect a JSON object with fields `subjects`, `predicate`, `objects`, and `negativity` from standard input.
...
Requires method `String.codePointCompare` to compare strings by Unicode Code Point. See [this TC39 proposal](http://tc39.es/proposal-compare-strings-by-codepoint/) for an implementation. For JavaScript in the browser use the Web Cryptograpghy API function [`webcrypto.subtle.digest`](https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/digest) instead of Node module `crypto`.

~~~js
{{< include implementation/javascript.js >}}
~~~

### Java {-}

Requires a package to parse JSON such as Jackson. Calculation of SHA-256 hash can be done with built in `MessageDigest` or with a more convenient library such as Apache Commons Codecs:

~~~java
{{< include implementation/MappingSamenessIdentifier.java >}}
~~~

### Command line {-}

Requires additional tool [jq](https://jqlang.org/).
...
Mappings can be expressed in the [Simple Standard for Sharing Ontological Mappings (SSSOM)](https://mapping-commons.github.io/sssom/) in a tabular format:
...
Negative mappings can be specified in SSSOM with the [`predicate_modifier`](https://w3id.org/sssom/predicate_modifier) column:
